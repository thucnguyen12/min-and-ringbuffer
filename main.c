...
/* USER CODE BEGIN PV */
struct min_context mContext;
struct min_context mContext2;
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    HAL_Delay (100);
    ringBufferInit (&ringBuf1, tempbuff);
    min_init_context(&mContext, 0); // context voi 2 port usart 2 va 6
    min_send_frame (&mContext, 1, (uint8_t *)("12345"), strlen("12345"));
  ...
  while (1)
  {
      min_send_frame (&mContext, 1, (uint8_t *)("hello"), strlen("hello"));
      HAL_Delay (1000);
      while (getByteFromRingBufferAvailableToRead(&ringBuf1))
      {
        uint8_t read = readFromRingBuffer (&ringBuf1);
        //putChar (USART6 ,read);
        rx_byte(&mContext, read);
      }        
    /* USER CODE END WHILE */
...
/* USER CODE BEGIN 4 */
/************** handler (callback from min.c ) handle data loaded *********/
